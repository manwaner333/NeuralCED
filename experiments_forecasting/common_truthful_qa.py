...
def _evaluate_metrics_forecasting(model_name, dataloader, model, x_times, times, loss_fn, device, kwargs):
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            *coeffs, x, y, xy, x_lengths, y_lengths, question_ids, labels = batch
            batch_size = y.size(0)

            real_y = xy
            if model_name in ('naivesde', 'neurallsde'):
                pred_y = model(x_times, times, coeffs, **kwargs)
            elif model_name == 'ncde':
                pred_y = model(x_times, times, coeffs, x_lengths + y_lengths - 1, **kwargs)
            ...
            for i in range(len(x_lengths)):
                ...
                mask_pred[i, :x_length + y_length, :] = True

            mask_real = torch.zeros(real_y.shape[0], real_y.shape[1], real_y.shape[2], dtype=torch.bool).to(device)
            for i in range(len(x_lengths)):
                x_length = x_lengths[i]
                y_length = y_lengths[i]
                mask_real[i, :x_length + y_length, :] = True

            total_dataset_size += batch_size
            mask_pred_y = torch.where(mask_pred, pred_y, torch.tensor(0.0))
            mask_real_y = torch.where(mask_real, real_y, torch.tensor(0.0))
...
def _train_loop_forecasting(model_name, train_dataloader, val_dataloader,test_dataloader, model, x_times, times, optimizer, loss_fn, eval_fn, max_epochs,
    ...
    for epoch in tqdm_range:
        ...
        for batch in train_dataloader:
            ...
            with _SuppressAssertions(tqdm_range):
                ...
                *train_coeffs, train_x, train_y, train_xy, train_x_lengths, train_y_lengths, train_question_ids, labels = batch
                if model_name in ('naivesde', 'neurallsde'):
                    pred_y = model(x_times, times, train_coeffs, **kwargs)
                elif model_name == 'ncde':
                    pred_y = model(x_times, times, train_coeffs, train_x_lengths + train_y_lengths -1, **kwargs)
                # real_y =train_y
                real_y = train_xy
                ...
                for i in range(len(train_x_lengths)):
                    ...
                    y_length = train_y_lengths[i]
                    mask_pred[i, :x_length+y_length, :] = True

                # mask_real = torch.zeros(real_y.shape[0], real_y.shape[1], real_y.shape[2], dtype=torch.bool).to(device)
                # for i in range(len(train_y_lengths)):
                #     y_length = train_y_lengths[i]
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            ...
            train_metrics = _evaluate_metrics_forecasting(model_name, train_dataloader, model, x_times, times, eval_fn,  device, kwargs)
            val_metrics = _evaluate_metrics_forecasting(model_name, val_dataloader, model, x_times, times, eval_fn,  device, kwargs)
            test_metrics = _evaluate_metrics_forecasting(model_name, test_dataloader, model, x_times, times, eval_fn,  device,kwargs)
...
def main_forecasting(name, model_name, x_times, times, train_dataloader, val_dataloader, test_dataloader, device, make_model, max_epochs,
    ...
    # train function
    history = _train_loop_forecasting(model_name, train_dataloader, val_dataloader, test_dataloader, model, x_times, times, optimizer, loss_fn, eval_fn, max_epochs,
                                      writer, device, kwargs, step_mode)
