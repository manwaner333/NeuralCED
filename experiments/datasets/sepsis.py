...
data_loc = base_base_loc / 'sepsis/training_setA/training'
...
def _process_data(static_intensity, time_intensity):
    ...
    X_static = []
    y = []
    for filename in os.listdir(base_loc):  # base_loc
...
def get_data():
    ...
    with open("test_data_for_ode.bin", 'rb') as f:
        keys_val = pickle.load(f)
    x = keys_val['x']
    x_reduce = []
    for ele in x:
        ele_reduce = pca.fit_transform(ele)
        x_reduce.append(ele_reduce)
    x = x_reduce
    y = np.array(keys_val['y'])
    max_length = 0
    cases_number = len(x)
    hidden_number = len(x[0][0])
    for ele in x:
        if max_length < len(ele):
            max_length = len(ele)
    x_new = np.full((cases_number, max_length, hidden_number), 0.0)
    # x_new = np.array([np.pad(sublist, (0, max_length - len(sublist)), 'constant', constant_values=(-1)) for sublist in x])
    t = torch.linspace(0., (max_length-1)*1.0, max_length)
    for i, sublist in enumerate(x):
        for j, item in enumerate(sublist):
            x_new[i, j, :] = item
    x_sub_tensor = torch.from_numpy(x_new)
    x_tensor = torch.cat((t.unsqueeze(0).repeat(cases_number, 1).unsqueeze(2), x_sub_tensor), dim=2)
    y_tensor = torch.from_numpy(y)
    return t.float(), x_tensor.float(), y_tensor.float()
"""
