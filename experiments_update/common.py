...
def _train_loop(train_dataloader, test_dataloader, model, times, optimizer, loss_fn, max_epochs, num_classes, device,
                kwargs, step_mode):
    model.train()
    best_model = model
    ...
    for epoch in tqdm_range:
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            ...
            history.append(_AttrDict(epoch=epoch, train_metrics=train_metrics, val_metrics=test_metrics))
...
def main(name, times, train_dataloader, test_dataloader, device, make_model, num_classes, max_epochs,
    ...
    history = _train_loop(train_dataloader, test_dataloader, model, times, optimizer, loss_fn, max_epochs,
