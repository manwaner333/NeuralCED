...
base_loc = base_base_loc / 'neg_city'
loc_Azip = base_loc / 'training_setA.zip'
loc_Bzip = base_loc / 'training_setB.zip'
data_loc = base_base_loc / 'neg_city/training_setA/training'


def download():
    pca = PCA(n_components=512)
    with open("datasets/data/neg_city/answer_neg_city.bin", 'rb') as f:
    ...
    #     x_res.append(ele[-1])
    #
    # np.save("datasets/x_city.npy", x_res)
    # np.save("datasets/y_city.npy", y)
...
def get_data(static_intensity, time_intensity, batch_size):
    ...
    loc = base_base_loc / ('neg_city' + ('_staticintensity' if static_intensity else '_nostaticintensity') + ('_timeintensity' if time_intensity else '_notimeintensity'))
