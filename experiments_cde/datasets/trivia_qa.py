...
base_loc = base_base_loc / 'trivia_qa'
loc_Azip = base_loc / 'training_setA.zip'
loc_Bzip = base_loc / 'training_setB.zip'
data_loc = base_base_loc / 'trivia_qa/training_setA/training'


def download():
    pca = PCA(n_components=512)
    with open("datasets/data/trivia_qa/answer_trivia_qa.bin", 'rb') as f:
...
def get_data(static_intensity, time_intensity, batch_size):
    ...
    loc = base_base_loc / ('trivia_qa' + ('_staticintensity' if static_intensity else '_nostaticintensity') + ('_timeintensity' if time_intensity else '_notimeintensity'))
