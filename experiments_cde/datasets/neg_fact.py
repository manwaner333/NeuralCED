...
base_loc = base_base_loc / 'neg_fact'
loc_Azip = base_loc / 'training_setA.zip'
loc_Bzip = base_loc / 'training_setB.zip'
data_loc = base_base_loc / 'neg_fact/training_setA/training'


def download():
    pca = PCA(n_components=512)
    with open("datasets/data/neg_fact/answer_neg_fact.bin", 'rb') as f:
    ...
    #     x_res.append(ele[-1])
    #
    # np.save("datasets/x_fact.npy", x_res)
    # np.save("datasets/y_fact.npy", y)
...
def get_data(static_intensity, time_intensity, batch_size):
    ...
    loc = base_base_loc / ('neg_fact' + ('_staticintensity' if static_intensity else '_nostaticintensity') + ('_timeintensity' if time_intensity else '_notimeintensity'))
